...
import { getHookProvider } from '../../server/src/providerRegistry.js';
...
export function getProjectDirPath(cwd?: string, providerId: string = 'claude'): string {
  ...
  if (dirs.length === 0) {
    throw new Error(`${providerId} provider getSessionDirs returned no directories`);
  }
...
export async function launchNewTerminal(
  ...
  const terminalNamePrefix = provider.displayName || CLAUDE_TERMINAL_NAME_PREFIX;
  ...
  if (!launch) {
    throw new Error(`${providerId} provider buildLaunchCommand is not implemented`);
  }
  terminal.sendText([launch.command, ...launch.args].join(' '));

  const projectDir = getProjectDirPath(cwd, providerId);
